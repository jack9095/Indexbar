package stick.head.recycler.stick;
...
public class StickHeaderDecoration1 extends RecyclerView.ItemDecoration {
     ...
     * 进行一些容错检查
     */
    public StickHeaderDecoration1(RecyclerView recyclerView) {
        ...
        for (int i = position; i >= 0; i--) {
            if (stickHeaderInterface.isStick(i)) {
                ...
                if (top < inflate.itemView.getMeasuredHeight() && top > 0) {
                    canvas.translate(0, top - measureHeight);
                }
                inflate.itemView.draw(canvas);
                canvas.restore();
                return;
            }
        }
